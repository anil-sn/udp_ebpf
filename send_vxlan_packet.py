...
def send_packet_on_interface(interface, packet_data):
    ...
    try:
        ...
        # Send the packet
        s.send(packet_data)
        print(f"✅ Sent {len(packet_data)} bytes on {interface}")
...
"""
    ...
    print(f"📡 Sending packet on interface: {interface}")
    ...
    try:
        ...
        if hasattr(pkt, 'src'):
            print(f"   └─ Src MAC: {pkt.src}")
            print(f"   └─ Dst MAC: {pkt.dst}")
        if pkt.haslayer(IP):
            ip_layer = pkt[IP]
            print(f"   └─ Outer IP: {ip_layer.src} → {ip_layer.dst}")
    ...
    if send_packet_on_interface(interface, packet_data):
        print(f"✅ SUCCESS: Packet sent on {interface}")
        print(f"🔍 Monitor {interface} and ens6 to see the packet processing:")
        print(f"   sudo tcpdump -i {interface} -n -X")
        print(f"   sudo tcpdump -i ens6 -n -X")
    else:
        print("❌ FAILED: Could not send packet")
        sys.exit(1)
